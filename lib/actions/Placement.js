...
class Placement {
    ...
    async placeBuildingBlock(position) {
        const { referenceBlock, faceVector } =
            this.settings.utils.getReferenceBlockAndFaceVector(this.bot, position);
    ...
    buildUp() {
        ...
        return new Promise((resolve, reject) => {
            (async () => {
                ...
                await this.settings.utils.equipBuildingBlock.equipBuildingBlock(this.bot, this.settings);
                await this.bot.waitForTicks(1);
