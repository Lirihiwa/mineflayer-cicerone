...
class Digging {
    ...
    async digBlock(position) {
        ...
        try {
            const tool = this.settings.utils.harvestTool(this.bot, block, this.settings);
            if (tool) {
    ...
    /**
     * Breaks the block underneath itself.
     *
     * @returns {Promise<void>}
     * */
    async digDown() {
        const blockPosition = this.bot.entity.position.floored().offset(0, -1, 0);
        await this.digBlock(blockPosition);
    }
}
