...
class CiceroneSettings {
     ...
     *  to 1.1.
     * @param {number} [option.minDurability] - Minimal durability value for a tool capable of
     * digging. Defaults to 5.
    ...
    constructor({
        ...
        minDurability = DEFAULT_MIN_DURABILITY,
    } = {}) {
        ...
        this.setMinDurability(minDurability);

        this.utils = {
            harvestTool: ToolUtils.harvestTool,
            getReferenceBlockAndFaceVector: PlacementUtils.getReferenceBlockAndFaceVector,
            equipBuildingBlock: PlacementUtils.equipBuildingBlock,
        };
    }
    ...
    /**
     * @param {number} minDirability
     * @throws {CiceroneError} If minDurability is not a positive number or greater than 100.
     */
    setMinDurability(minDirability) {
        if (
            typeof minDirability !== 'number' ||
            !Number.isFinite(minDirability) ||
            minDirability <= 0 ||
            minDirability > 100
        ) {
            throw new CiceroneError('buildUpThreshold must be a positive number.');
        }
        this._minDurability = minDirability;
    }
