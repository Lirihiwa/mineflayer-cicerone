...
class PathElement {
     ...
     * */
    constructor(position, type, state = null) {
    ...
    async run(actionRegistry) {
        try {
            ...
            throw new CiceroneError(`Unable to run [${this.getInfo()}]: ${e.message}`, {
                cause: e,
            });
        }
    }
    ...
    async reverse(actionRegistry) {
        try {
            ...
            throw new CiceroneError(`Unable to reverse [${this.getInfo()}]: ${e.message}`, {
                cause: e,
            });
        }
    }
