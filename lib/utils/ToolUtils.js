    ...
    static harvestTool(bot, block, settings) {
        ...
        const tools = bot.inventory.items().filter((item) => {
            ...
            if (item.maxDurability) {
                const remainingDurability = ToolUtils.getDurability(item);
        ...
        let tool;
        if (isOre) {
            tool = tools.find((item) => ToolUtils.getEnchantments(item).silk_touch);
        } else {
            tool = tools.find((item) => ToolUtils.getEnchantments(item).fortune);
        }
