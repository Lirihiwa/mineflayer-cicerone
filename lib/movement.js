...
module.exports = (bot) => {
    ...
    bot.cicerone.goto = async (vec, options = {}) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            tickListener = () => {
                ...
                if (distance < 0.4 && bot.entity.onGround) {
     ...
     * */
    bot.cicerone.jumpOn = async (vec, options = {}) => {
        while (!bot.entity.onGround) {
            await bot.waitForTicks(2);
        }

        await bot.lookAt(vec.offset(0.5, 0.5, 0.5));
        bot.setControlState('jump', true);

        while (vec.y - bot.entity.position.y < 0.3) {
            console.log(vec.y - bot.entity.position.y);
